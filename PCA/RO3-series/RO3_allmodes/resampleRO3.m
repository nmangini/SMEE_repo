...
tic
% Load data
load('vectors_RO3-series.mat')
load('time_RO3-series.mat')
load('injection_RO3-series.mat')
load('Tinjection_RO3-series.mat')

% Variables and matrix set up
fs = 16384;                 % New sampling frequency
a = zeros(length(RO3T),20);
for i = 1:20
    a(:,i) = RO3T;            % Time matrix to be added to the PC matrix
end
WF = vectorsPC;
...
MDCinj = zeros(length(RvectorsPC),20);
...
% Save files
PC_savefile = 'RvectorsPC_RO3-series';
save(PC_savefile,'RvectorsPC')
MDC_savefile = 'MDC_RO3-series';
save(MDC_savefile,'MDCinj')
time_savefile = 'resampledtime_RO3-series';
