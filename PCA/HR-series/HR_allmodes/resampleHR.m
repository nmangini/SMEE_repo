...
tic
% Load data
load('vectors_HR-series.mat')
load('time_HR-series.mat')
load('injection_HR-series.mat')
load('Tinjection_HR-series.mat')

% Variables and matrix set up
fs = 16384;                 % New sampling frequency
a = zeros(length(HRT),15);
for i = 1:15
    a(:,i) = HRT;            % Time matrix to be added to the PC matrix
end
WF = vectorsPC;
...
MDCinj = zeros(length(RvectorsPC),15);
...
% Save files
PC_savefile = 'RvectorsPC_HR-series';
save(PC_savefile,'RvectorsPC')
MDC_savefile = 'MDC_HR-series';
save(MDC_savefile,'MDCinj')
time_savefile = 'resampledtime_HR-series';
